...
public class MessageDAO {
    ...
    public void insertStudentReply(int reply_to_id, boolean isReplied, int sender_student_id, int recipient_teacher_id, java.sql.Timestamp sent_date, String subject, 
    String body) throws Exception {
        ...
        try {
            ...
            stmt.setInt(4, sender_student_id);
            stmt.setInt(5, recipient_teacher_id);
    ...
    public void hasUnreadMessagesStudent(int student_id) throws Exception {
        Connection con = null;
        String sql = "SELECT * FROM message WHERE recipient_student_id = ? AND is_read = 0;";
    ...
    public Map<Teacher, List<Message>> getStudentConversarions(int studentId) throws Exception {
        Connection con = null;
        Map<Teacher, List<Message>> conversations = new HashMap<>();

        TeacherDAO teacherDAO = new TeacherDAO();

        String query = "SELECT * FROM message " +
        "WHERE (sender_student_id = ? OR recipient_student_id = ?) " +
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Fetch the teacher object based on the sender or recipient
                int teacherId = (message.getSenderType().equals("teacher")) ? message.getSenderId() : message.getRecipientId();
                Teacher teacher = teacherDAO.getTeacherDetails(teacherId); 

                // Add the message to the correct student's conversation
                if (!conversations.containsKey(teacher)) {
                    conversations.put(teacher, new ArrayList<>());
                }
                conversations.get(teacher).add(message);
            }
    ...
    public void insertTeacherMessage(boolean isReplied, String sender_type, int sender_teacher_id, int recipient_student_id, java.sql.Timestamp sent_date,
    String subject, String body) throws Exception {
        Connection con = null;
        String sql =
        "INSERT INTO message (reply_to, is_replied, sender_type, sender_teacher_id, sender_student_id, recipient_teacher_id, recipient_student_id, sent_date, message_subject, message_body, is_read) " +
        ...
        try {
            ...
            stmt = con.prepareStatement(sql);
            stmt.setInt(1, isReplied ? 1 : 0);
            stmt.setString(2,sender_type);
            stmt.setInt(3, sender_teacher_id);
            stmt.setInt(4, recipient_student_id);
            stmt.setTimestamp(5, sent_date);
            stmt.setString(6, subject);
            stmt.setString(7, body);
            stmt.executeUpdate();
            ...
            throw new Exception("Error inserting student's message: " + e.getMessage());
        } finally {
            try {
                db.close();
            } catch(Exception e) {
    ...
    public void hasUnreadMessagesTeacher(int teacher_id) throws Exception {
        Connection con = null;
        String sql = "SELECT * FROM message WHERE recipient_teacher_id = ? AND is_read = 0;";
